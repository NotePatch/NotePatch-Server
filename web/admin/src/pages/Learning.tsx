...
export function LearningDetailPage() {
  ...
  return <section className="page">
    ...
    <div className="detail-grid">
      ...
      <section className="panel wide"><h2>最新闪卡组</h2>{deckDetail.data ? <table><thead><tr><th>排序</th><th>正面</th><th>背面</th><th>权重</th><th>推荐原因</th></tr></thead><tbody>{deckDetail.data.cards.map((card) => <tr key={card.id}><td>{card.rank}</td><td>{card.front}</td><td>{card.back}</td><td>{card.priority_score.toFixed(2)}</td><td><code>{card.review_hint.primary.code}</code><div className="subtext">{Object.entries(card.review_hint.primary.params).map(([key, value]) => key + "=" + String(value)).join(" · ") || card.review_hint.data_quality}</div></td></tr>)}</tbody></table> : <div className="empty-state"><Layers3 size={28}/><p>暂无持久化闪卡组。</p></div>}</section>
